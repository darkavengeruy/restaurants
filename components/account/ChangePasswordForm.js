...
export default function ChangePasswordForm({    
    ...
    const onSubmit = async () => {
      ...
      if (!resultUpdatePassword.statusResponse) {  
        setErrorNewPassword(" Hubo un problema cambiando la contraseña, porfavor intente mas tarde.");
        return;
      }
    ...
    const validateForm = () => {
      ...
      if (size(newPassword) < 6) {
        setErrorNewPassword(
          "Debes ingresar una nueva contraseña de al menos 6 caracteres.");
        isValid = false;
      }

      if (size(confirmPassword) < 6) {
        setErrorConfirmPassword(
          "Debes ingresar una nueva confirmacion de tu contraseña de al menos 6 caracteres.");
        isValid = false;
      }
